...
def integrate(integrated_f, base_f, preserved_algorithms, added_algorithms):
    ...
    for tup, _ in original.groupby(by=groupby, sort=False):
        ...
        if not _integrated.shape[0]:
            for _, record in _original.iterrows():
                if record["Algorithm"].lower() not in preserved_algorithms and record["Algorithm"].lower() in added_algorithms:
            ...
            for _, record in _integrated.iterrows():
                if record["Algorithm"].lower() not in preserved_algorithms and record["Algorithm"].lower() in added_algorithms:
                    records.append(record)
        for _, record in _original.iterrows():
            if record["Algorithm"].lower() in preserved_algorithms:
                records.append(record)
...
ap.add_argument('-b', '--base', help="Base.original file", type=str)
...
while idx < len(all_integrated):
    preserved, curr_base = integrate(all_integrated[idx], curr_base, preserved, added)
    idx += 1
